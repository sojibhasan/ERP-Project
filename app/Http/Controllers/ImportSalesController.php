...
class ImportSalesController extends Controller
{
	...
	public function index()
	{
		...
		if (!$this->moduleUtil->hasThePermissionInSubscription($business_id, 'import_sale')) {
            abort(403, 'Unauthorized action.');
        }
	...
	private function __importSales($formated_data, $business_id, $location_id)
	{
		...
		foreach ($formated_data as $data) {
			...
			foreach ($data as $line_data) {
				...
				$sale_import_date = Business::where('id', $business_id)->first()->sale_import_date;

				//transaction date is older then permmited date reurn false
				if ($this_date < $sale_import_date) {
					$output = [
						'success' => false,
						'msg' => __('lang_v1.sale_import_older_date_error')
					];
				...
				$temp = [
					...
					'quantity' => $this->productUtil->num_uf($line_data['quantity']),
					'unit_price' => $this->productUtil->num_uf($unit_price),
					'unit_price_inc_tax' => $this->productUtil->num_uf($line_data['unit_price']),
