...
class ImportProductsController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if ($request->hasFile('products_csv')) {
                ...
                foreach ($imported_data as $key => $value) {
                    ...
                    if ($product_array['type'] == 'single') {
                        ...
                        //Add  Stock Type
                        $account_name = trim($value[35]);
                        $stock_account = Account::where('name', $account_name)->where('business_id', $business_id)->first();
                        if (!empty($account_name) && !empty($stock_account)) {
                            $product_array['stock_type'] =  $stock_account->id;
                        } else {
                            $is_valid =  false;
                            $error_msg = "Invalid value for Stock Type in row no. $row_no";
                            break;
                        }
                        //Opening stock
                        if (!empty($value[20]) && $enable_stock == 1) {
                            $transaction_date_temp = \PhpOffice\PhpSpreadsheet\Shared\Date::excelToDateTimeObject($value[36]);
                            $product_array['opening_stock_details']['transaction_date'] = !empty($transaction_date_temp) ? Carbon::parse($transaction_date_temp)->format('Y-m-d') : date('Y-m-d');
                ...
                if (!empty($formated_data)) {
                    foreach ($formated_data as $index => $product_data) {
                        ...
                        if (!empty($product_data['opening_stock_details'])) {
                            $transaction_date = $product_data['opening_stock_details']['transaction_date'];
                        ...
                        if ($product->type == 'single') {
                            ...
                            if (!empty($opening_stock)) {
                                $os_transaction = $this->addOpeningStock($opening_stock, $product, $business_id, $transaction_date);
                                $total_opening_stock += $os_transaction->final_total;
                            }
                            ...
                            if (!empty($value[20]) && $enable_stock == 1) {
                                $this->addOpeningStockForVariable($variation_data, $product, $business_id, $transaction_date);
                            }
                        }
                    }
                }
            ...
            if (!empty($total_opening_stock)) {
                $opening_balance_equity_id = $this->transactionUtil->account_exist_return_id('Opening Balance Equity Account');
                $this->transactionUtil->createAccountTransaction($os_transaction, 'credit', $opening_balance_equity_id, $total_opening_stock);
            }
     ...
     * @return obj $product
     */
    private function addOpeningStock($opening_stock, $product, $business_id, $transaction_date)
    {
        $user_id = request()->session()->get('user.id');
        ...
        if ($product->enable_stock == 1 && !empty($product->stock_type)) {
    ...
    private function addOpeningStockForVariable($variations, $product, $business_id, $transaction_date)
    {
        $user_id = request()->session()->get('user.id');
        ...
        if (isset($variations['variations'][0]['opening_stock'])) {
            ...
            foreach ($variations['variations'] as $variation_os) {
                ...
                if ($product->enable_stock == 1 && $product->stock_type) {
                    $account_transaction_data = [
                        'amount' => $transaction->final_total,
                        'account_id' => $product->stock_type,
                        'type' => 'debit',
