...
<section class="content no-print">
	...
	<div class="box box-solid">
		<div class="box-body">
			<div class="row">
				<div class="col-sm-4">
					<div class="form-group">
						...
						{!! Form::text('invoice_no', !empty($temp_data->invoice_no) ? $temp_data->invoice_no :(!empty($sell->return_parent->invoice_no) ?
				...
				<div class="col-sm-3">
					<div class="form-group">
						...
						<div class="input-group">
							...
							@php
							$transaction_date =!empty($temp_data->transaction_date) ? $temp_data->transaction_date : (!empty($sell->return_parent->transaction_date) ?
							...
							<tr>
								...
								<td>
									<input type="text" name="products[{{$loop->index}}][quantity]"
										value="{{(!empty($temp_data->products[$i])?$temp_data->products[$i]->quantity:(!empty($sell_line->quantity_returned)?$sell_line->quantity_returned:0))}}"
									...
									<input name="products[{{$loop->index}}][unit_price_inc_tax]" type="hidden"
										class="unit_price" value="{{@num_format(!empty($temp_data->products[$i])?str_replace(',', '', $temp_data->products[$i]->unit_price_inc_tax):(!empty($sell_line->unit_price_inc_tax)?$sell_line->unit_price_inc_tax:0))}}">
									<input name="products[{{$loop->index}}][sell_line_id]" type="hidden"
										value="{{!empty($temp_data->products[$i])?$temp_data->products[$i]->sell_line_id:$sell_line->id}}">
								</td>
			...
			<div class="row">
				@php
				$discount_type = !empty($temp_data->discount_type)?$temp_data->discount_type:(!empty($sell->return_parent->discount_type) ? $sell->return_parent->discount_type : $sell->discount_type);
				$discount_amount = !empty($temp_data->discount_amount)?$temp_data->discount_amount:(!empty($sell->return_parent->discount_amount) ? $sell->return_parent->discount_amount : $sell->discount_amount);
				@endphp
				...
				<div class="col-sm-4">
					<div class="form-group">
						...
						'percentage' => __( 'lang_v1.percentage' )], $discount_type, ['class' => 'form-control']); !!}
					</div>
				</div>
				<div class="col-sm-4">
					<div class="form-group">
						{!! Form::label('discount_amount', __( 'purchase.discount_amount' ) . ':') !!}
						{!! Form::text('discount_amount', @num_format(str_replace(',', '',$discount_amount)), ['class' => 'form-control
						input_number']); !!}
					</div>
				</div>
			</div>
			...
			<div class="row">
				<div class="col-sm-12">
					<button type="submit" id="sell_return_submit"
...
<script type="text/javascript">
	$(document).ready( function(){
		...
		$('#sell_return_submit').click(function(e){
			e.preventDefault();
	...
	@if(auth()->user()->can('unfinished_form.sale'))
	setInterval(function(){ 
		$.ajax({
                method: 'POST',
                url: '{{action("TempController@saveSaleReturnTemp")}}',
                dataType: 'json',
                data: $('#sell_return_form').serialize(),
	...
	@if(!empty($temp_data))
		swal({
			...
			if(sure){
				window.location.href = "{{action('TempController@clearData', ['type' => 'sale_return_data'])}}";
			} 
		});
	@endif
	@endif
