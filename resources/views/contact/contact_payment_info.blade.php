...
@if( $contact->type == 'customer' || $contact->type == 'both')
    ...
    <strong class=" hide">@lang('contact.total_sale_due')</strong>
    <p class="text-muted hide">
    <span class="display_currency" data-currency_symbol="true">
    <?php /* {{ $contact->total_invoice - $contact->invoice_received }} */?>
    {{ $contact->total_sale_due - $contact->invoice_received }}</span>
    </span>
    </p>
@endif
@if(!empty($contact->opening_balance) && $contact->opening_balance != '0.00')
    ...
    <strong class=" hide">@lang('lang_v1.opening_balance_due')</strong>
    <p class="text-muted">
    <span class="display_currency hide" data-currency_symbol="true">
    <?php /* {{ $contact->opening_balance - $contact->opening_balance_paid }} */ ?>
    {{$contact->total_invoice + $contact->opening_balance }}</span>
    </span>
    </p>
@endif