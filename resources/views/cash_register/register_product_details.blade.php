    ...
    <table class="table">
      ...
      @foreach($details['product_details'] as $detail)
        <tr>
          ...
          <td>
            {{ ($detail->brand_name) ? $detail->brand_name : 'No Brand'}}
          </td>
          <td>
            @if(isset($business) && $business->quantity_precision > 0)
                {{ number_format( $detail->total_quantity, $business->quantity_precision, '.','') }}
            @else
