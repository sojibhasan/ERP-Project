  ...
  <div class="row invoice-info">
    <div class="col-sm-4 invoice-col">
      ...
      <address>
        ...
        !empty($purchase->contact->country))
    ...
    <div class="col-sm-4 invoice-col">
      ...
      <address>
        ...
        @endif
        @if(!empty($purchase->location->city) || !empty($purchase->location->state) ||
      ...
      <div class="table-responsive">
        <table class="table bg-gray">
          ...
          <tr>
            ...
            <td>
              @if(!empty($purchase_line->product->name)) {{ $purchase_line->product->name }} @endif
              @if( $purchase_line->product->type == 'variable')
              @if(!empty($purchase_line->variations->product_variation->name))-
              {{ $purchase_line->variations->product_variation->name}} @endif
              @if(!empty($purchase_line->variations->name))- {{ $purchase_line->variations->name}} @endif
              @endif
            </td>
            <td><span class="display_currency" data-is_quantity="true"
              ...
              @if(!empty($purchase_line->sub_unit)) {{$purchase_line->sub_unit->short_name}} @else
              {{$purchase_line->product->unit->short_name}} @endif</td>
            @if ($enable_free_qty)
            <td><span class="display_currency" data-is_quantity="true"
            ...
            <td class="no-print"><span class="display_currency"
                data-currency_symbol="true">{{ $purchase_line->purchase_price }}</span></td>
            <td class="no-print"><span class="display_currency"
                data-currency_symbol="true">{{ $purchase_line->quantity * $purchase_line->purchase_price }}</span></td>
            ...
            <td><span class="display_currency"
                data-currency_symbol="true">{{ $purchase_line->purchase_price_inc_tax }}</span></td>
            @php
