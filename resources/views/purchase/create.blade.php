				...
				<table class="table table-condensed table-bordered table-th-green text-center table-striped"
					...
					<thead>
						<tr>
							...
							<th style="width: 10%;">@lang( 'purchase.purchase_quantity' )</th>
							<th style="width: 10%;">@lang( 'product.units' )</th>
							@if ($enable_free_qty)
							<th style="width: 5%;">@lang( 'purchase.free_qty' )</th>
							@endif
							<th style="width: 10%;">@lang( 'purchase.available_qty' )</th>
							<th style="width: 10%;">@lang( 'lang_v1.unit_cost_before_discount' )</th>
							<th style="width: 8%;">@lang( 'lang_v1.discount_percent' )</th>
							<th style="width: 8%;">@lang( 'purchase.unit_cost_before_tax' )</th>
							<th class="{{$hide_tax}}"  style="width: 10%;">@lang( 'purchase.subtotal_before_tax' )</th>
							<th class="{{$hide_tax}}"  style="width: 10%;">@lang( 'purchase.product_tax' )</th>
							<th class="{{$hide_tax}}"  style="width: 10%;">@lang( 'purchase.net_cost' )</th>
							<th  style="width: 10%;">@lang( 'purchase.line_total' )</th>
				...
				success: function(result) {
					$(result)
						...
						.each(function() {
							...
							update_row_price_for_exchange_rates(row);

							update_inline_profit_percentages(row);

							update_table_totals();
							update_grand_totals();
							update_table_sr_numbers();
...
@endif
<script>
	function update_row_price_for_exchange_rates(row) {
...
function update_inline_profit_percentages(row) {
...
function update_table_sr_numbers() {
    var sr_number = 1;
...
$('.business_location_id').change(function(){
		...
		$.ajax({
			...
			success: function(result) {
				...
				$('.payment_types_dropdown').val({{$cash_account_id}}).trigger('change');
			},
		});
		getInvoice();
	...
	function getInvoice() {
		$.ajax({
			...
			success: function(result) {
				...
				$('.payment_types_dropdown').val({{$cash_account_id}});
