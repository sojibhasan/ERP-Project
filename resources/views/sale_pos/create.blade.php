					...
					<div class="@if(!empty($commission_agent)) col-sm-4 @else col-sm-6 @endif">
						<div class="form-group" style="width: 100% !important">
							<div class="input-group">
								...
								[], !empty($temp_data->contact_id)?$temp_data->contact_id:null, ['class' =>
								'form-control mousetrap', 'id' => 'customer_id', 'placeholder' =>
...
<!-- Call restaurant module if defined -->
@if(in_array('tables' ,$enabled_modules) || in_array('modifiers' ,$enabled_modules) ||
...
$('#customer_id').change(function(){
	customer_name = $('#customer_id :selected').text();
	...
	$.ajax({
		...
		success: function(result) {
			...
			let due_amount_customer = parseFloat(result.due_amount.replace(',', ''));
			if(parseFloat(due_amount_customer) < 0){ //previous access amount customer excess
				due_amount_customer = due_amount_customer *  -1 ;
				$('input#was_customer_wallet').val(parseFloat(due_amount_customer));
        		$('span.customer_wallet').text(__currency_trans_from_en(parseFloat(due_amount_customer), true));
				__write_number($('input#total_paying_input'), parseFloat(due_amount_customer));
        		$('span.total_paying').text(__currency_trans_from_en(parseFloat(due_amount_customer), true));
			}else{
				__write_number($('input#was_customer_wallet'), 0);
        		$('span.customer_wallet').text(__currency_trans_from_en(0, true));
			}
...
$('#add_to_customer_wallet').click(function(){
	...
	$('span.customer_wallet').text(__currency_trans_from_en(parseFloat(was_customer_wallet + change_return), true));
})
