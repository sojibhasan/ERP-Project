...
<section class="content">
	...
	{!! Form::open(['url' => action('SellPosController@update', $transaction->id ), 'method' => 'put', 'id' =>
	'edit_sell_form' ]) !!}

	{!! Form::hidden('location_id', $transaction->location_id, ['id' => 'location_id', 'data-receipt_printer_type' =>
	!empty($location_printer_type) ? $location_printer_type : 'browser']); !!}

	<input type="hidden" name="price_later" id="price_later" value="{{$transaction->price_later}}">
	<div class="row">
		<div class="col-md-12 col-sm-12">
			...
			<div class="col-md-4 col-sm-6">
				<div class="form-group">
					<div class="input-group">
						...
						!!}
						{!! Form::text('price_group_text', $transaction->price_group->name, ['class' => 'form-control',
						'readonly']); !!}
						<span class="input-group-addon">
			...
			<div class="col-md-4 col-sm-6">
				<div class="form-group">
					<div class="input-group">
						...
						{!! Form::text('types_of_service_text', $transaction->types_of_service->name, ['class' =>
						'form-control', 'readonly']); !!}

						{!! Form::hidden('types_of_service_id', $transaction->types_of_service_id, ['id' =>
						'types_of_service_id']) !!}
					...
					<small>
						<p class="help-block @if(empty($transaction->selling_price_group_id)) hide @endif"
							id="price_group_text">@lang('lang_v1.price_group'):
							<span>@if(!empty($transaction->selling_price_group_id)){{$transaction->price_group->name}}@endif</span>
						</p>
					</small>
				</div>
			</div>
			...
			<div class="col-md-4 pull-right col-sm-6">
				<div class="checkbox">
					<label>
						{!! Form::checkbox('is_recurring', 1, $transaction->is_recurring, ['class' => 'input-icheck',
						'id' => 'is_recurring']); !!} @lang('lang_v1.subscribe')?
			...
			<div class="@if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
				<div class="form-group">
					...
					<div class="input-group">
						...
						[], null, ['class' => 'form-control mousetrap', 'id' => 'customer_id', 'placeholder' => 'Enter
			...
			<div class="col-md-3">
				<div class="form-group">
					<div class="multi-input">
						...
						<br />
						{!! Form::number('pay_term_number', $transaction->pay_term_number, ['class' => 'form-control
			...
			<div class="@if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
				<div class="form-group">
					...
					<div class="input-group">
						...
						{!! Form::text('transaction_date', @format_datetime($transaction->transaction_date), ['class' => 'form-control',
						'readonly', 'required']); !!}
					</div>
				</div>
			</div>
			...
			<div class="@if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif">
				<div class="form-group">
					...
					__('lang_v1.quotation')], $status, ['class' => 'form-control select2', 'placeholder' =>
			...
			<div class="col-sm-3">
				<div class="form-group">
					...
					{!! Form::select('invoice_scheme_id', $invoice_schemes, $default_invoice_schemes->id, ['class' =>
			...
			<div class="col-sm-10 col-sm-offset-1">
				...
				<div class="row">
					<a href="#"
						style="margin-top: 2px; background: @if($transaction->price_later == 1) blue @else rgb(202, 132, 2) @endif; color: #fff; margin-right: 50px;"
					...
					<table class="table table-condensed table-bordered table-striped table-responsive" id="pos_table">
						<thead>
							<tr>
								...
								<th class="price_later_td @if($transaction->price_later != 1) hide @endif">
									@lang('lang_v1.purchase_price')</th>
								<th>@lang('sale.tax')</th>
								<th class="text-center">
						...
						<tbody>
							...
							'tax_dropdown' => $taxes, 'sub_units' => !empty($sell_line->unit_details) ?
							$sell_line->unit_details : [], 'price_later' => $transaction->price_later ])
							@endforeach
						</tbody>
					</table>
				</div>
			...
			<div class="col-md-4">
				<div class="form-group">
					...
					<div class="input-group">
						...
						__('lang_v1.percentage')], $transaction->discount_type , ['class' =>
						'form-control','placeholder' => __('messages.please_select'), 'required', 'data-default' =>
						'percentage']); !!}
					</div>
				</div>
			</div>
			<div class="col-md-4">
				<div class="form-group">
					...
					<div class="input-group">
						...
						{!! Form::text('discount_amount', @num_format($transaction->discount_amount), ['class' =>
						'form-control input_number', 'data-default' => $business_details->default_sales_discount]); !!}
					</div>
				</div>
			</div>
			...
			<div class="col-md-12 well well-sm bg-light-gray @if(session('business.enable_rp') != 1) hide @endif">
				...
				<div class="col-md-4">
					<div class="form-group">
						...
						<div class="input-group">
							...
							{!! Form::number('rp_redeemed_modal', $transaction->rp_redeemed, ['class' => 'form-control
							direct_sell_rp_input', 'data-amount_per_unit_point' =>
							session('business.redeem_amount_per_unit_rp'), 'min' => 0, 'data-max_points' =>
							!empty($redeem_details['points']) ? $redeem_details['points'] : 0, 'data-min_order_total' =>
				...
				<div class="col-md-4">
					<p><strong>@lang('lang_v1.redeemed_amount'):</strong> (-)<span
							id="rp_redeemed_amount_text">{{@num_format($transaction->rp_redeemed_amount)}}</span></p>
				</div>
			</div>
			<div class="clearfix"></div>
			<div class="col-md-4">
				<div class="form-group">
					...
					<div class="input-group">
						...
						{!! Form::select('tax_rate_id', $taxes['tax_rates'], $transaction->tax_id, ['placeholder' =>
						__('messages.please_select'), 'class' => 'form-control', 'data-default'=>
			...
			<div class="col-md-4">
				<div class="form-group">
					...
					<div class="input-group">
						...
						{!! Form::textarea('shipping_details',$transaction->shipping_details, ['class' =>
						'form-control','placeholder' => __('sale.shipping_details') ,'rows' => '1', 'cols'=>'30']); !!}
					</div>
				</div>
			</div>
			<div class="col-md-4">
				<div class="form-group">
					...
					<div class="input-group">
						...
						{!! Form::textarea('shipping_address', $transaction->shipping_address, ['class' =>
						'form-control','placeholder' => __('lang_v1.shipping_address') ,'rows' => '1', 'cols'=>'30']);
						!!}
					</div>
				</div>
			</div>
			<div class="col-md-4">
				<div class="form-group">
					...
					<div class="input-group">
						...
						{!!Form::text('shipping_charges',@num_format($transaction->shipping_charges),['class'=>'form-control
			...
			<div class="col-md-4">
				<div class="form-group">
					...
					{!! Form::select('shipping_status',$shipping_statuses, $transaction->shipping_status, ['class' =>
			...
			<div class="col-md-12">
				<div class="form-group">
					...
					{!! Form::textarea('sale_note', $transaction->additional_notes, ['class' => 'form-control', 'rows'
					=> 3]); !!}
				</div>
			</div>
...
@if($transaction->price_later == 1)
<script>
	
	$(document).ready(function () {
	$('.btn-price-later').click()
	console.log('asdf');
