...
<div class="table-responsive">
    <table class="table table-bordered add-product-price-table table-condensed {{$class}}">
        <tr>
          ...
          <th>@lang('product.profit_percent')  @if(!empty($help_explanations['margin_percentage'])) @show_tooltip($help_explanations['margin_percentage']) @endif</th>
        ...
        <tr>
          ...
          <td class="multiple_units units_list">
            {{-- appending field in product/create page  --}}
          </td>

          <td>
            <label><span class="dsp_label">@lang('product.exc_of_tax')</span></label>
            {!! Form::text('single_dsp', $default, ['class' => 'form-control input-sm dsp input_number', 'style' => 'margin-top:7px;', 'placeholder' => __('product.exc_of_tax'), 'id' => 'single_dsp', 'required']); !!}

            {!! Form::text('single_dsp_inc_tax', $default, ['class' => 'form-control input-sm hide input_number',  'style' => 'margin-top:7px;', 'placeholder' => __('product.inc_of_tax'), 'id' => 'single_dsp_inc_tax', 'required']); !!}
