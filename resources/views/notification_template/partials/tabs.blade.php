    ...
    <div class="tab-content">
        @foreach($templates as $key => $value)
            <div class="tab-pane @if($loop->index == 0) active @endif" id="cn_{{$key}}">
                ...
                @endif
            <div style="@if($request->segment(2) == 'sms-template') display:none; @endif">
            ...
            <div style="@if($request->segment(2) == 'email-template') display:none; @endif">
                <div class="form-group">
                ...
                @if($key == 'new_sale' || $key == 'customer_new_order' || $key == 'order_confirmed' || $key == 'customer_notify')
                    <div class="form-group">
                        <label class="checkbox-inline" style="@if($request->segment(2) == 'sms-template') display:none; @endif">
                            {!! Form::checkbox('template_data[' . $key . '][auto_send]', 1, $value['auto_send'], ['class' => 'input-icheck']); !!} @lang('lang_v1.autosend_email')
                        </label>
                        <label class="checkbox-inline" style="@if($request->segment(2) == 'email-template') display:none; @endif">
