...
@endphp
{!! Form::open(['url' => action('SampleMedicalProductController@store'), 'method' => 'post', 
    ...
    @component('components.widget', ['class' => 'box-primary'])
        ...
        @php
            ...
            $business_setting = App\Business::where('id', $business_id )->first();
            $sku_prefix =  $business_setting->sku_prefix;
            $last_sku = DB::table('products')->select('id','sku')->where('business_id', $business_id)->orderBy('id', 'desc')->first();
            if(empty($sku_prefix)){
              $sku_prefix = '';
            }
            if(empty($last_sku)){
              $next_sku = $sku_prefix.'-0001';
            }else{
              $sku = $last_sku->sku;
              $numbers = preg_replace('/[^0-9]/', '', $sku);
              $letters = preg_replace('/[^a-zA-Z]/', '', $sku);
              $numbers++;
              $numbers = sprintf("%04d", $numbers);
              $next_sku = '0001';
              if(empty($sku_prefix)){
                $next_sku = $numbers;
              }else{
                $next_sku = $sku_prefix .'-'.$numbers;
        ...
        <div class="col-sm-4">
          <div class="form-group">
            ...
            {!! Form::text('sku', $next_sku, ['class' => 'form-control',
