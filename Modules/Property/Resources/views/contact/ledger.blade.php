	...
	<table class="table table-striped @if(!empty($for_pdf)) table-pdf td-border @endif" id="ledger_table">
		<thead>
			<tr class="row-border">
				...
				<th>@lang('property::lang.block_nos')</th>
		...
		<tbody>
			...
			<tr>
				...
				@elseif(!empty($data->installment_no))
				<td class="row-border">{{@format_date($data->installment_date)}}</td>
				@else
				...
				<td>
					...
					@else
						...
						@elseif($data->transaction_type == 'property_sell')
						@lang('lang_v1.sell'): {{$data->invoice_no}}
							@if(!empty($data->income_type))
							<br><b>{{ucfirst($data->income_type)}}
							@endif
						@endif
						@if(!empty($data->installment_no))
								<br><b>@lang('property::lang.installment_no'):</b> {{$data->installment_no}}
						@endif
						@if(!empty($data->deleted_by))
				...
				<td>@if($data->transaction_type == 'purchase') @lang('lang_v1.purchase') @elseif($data->transaction_type
					...
					@elseif(!is_null($data->payment_option)) {{$data->payment_option}}
					@endif</td>
				...
				<td class="ws-nowrap">
					@php
						$balance_view = $contact->type == 'customer' ? $data->balance :$balance;		
					@endphp
					{{number_format(	$balance_view,  $currency_precision, session('currency')['decimal_separator'], session('currency')['thousand_separator'])}}
				</td>
