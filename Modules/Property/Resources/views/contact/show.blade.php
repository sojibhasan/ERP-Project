            ...
            <div class="nav-tabs-custom">
                <ul class="nav nav-tabs nav-justified">
                    ...
                    <li class="
                        ...
                        <a href="#sales_tab" data-toggle="tab" aria-expanded="true"><i class="fa fa-arrow-circle-up"
                                aria-hidden="true"></i> @if($contact->is_property) @lang('property::lang.properties_purchased') @else @lang( 'sale.sells') @endif</a>
                    </li>
                    @endif
                    <li class="
                ...
                <div class="tab-content" style="background: #fbfcfc;">
                    ...
                    <div class="tab-pane
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                @include('property::contact.partials.ledger_tab')
                            </div>
                        </div>
                    </div>
                    @if(in_array($contact->type, ['both', 'supplier']))
...
<div class="modal fade pay_contact_due_modal"  role="dialog" aria-labelledby="gridSystemModalLabel"></div>
@stop
@section('javascript')
<script type="text/javascript">
    ...
    $('#ledger_date_range_new, #ledger_block_id, #ledger_project_id').change( function(){
        get_contact_ledger();
    });
    get_contact_ledger();
    ...
    $('#contact_id').change( function() {
        if ($(this).val()) {
            window.location = "{{url('/property/contacts')}}/" + $(this).val();
        }
    });
});
...
function get_contact_ledger() {
    ...
    var start_date = '';
    var end_date = '';
    var project_id = $('select#ledger_project_id').val();
    var block_id = $('select#ledger_block_id').val();
    ...
    $.ajax({
        url: '/property/contacts/ledger?contact_id={{$contact->id}}&start_date=' + start_date + '&project_id=' + project_id + '&block_id=' + block_id + '&end_date=' + end_date,
        dataType: 'html',
...
<script>
    ...
    $(document).on('change', '#ledger_project_id', function () {
        $.ajax({
            method: 'get',
            url: '/property/property-blocks/get-block-dropdown/'+$(this).val(),
            data: {  },
            success: function(result) {
                if(result.success === 1){
                    $('#ledger_block_id').empty().append(result.data);
                }
            },
        });
    })
