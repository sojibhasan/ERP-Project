@extends('layouts.app')
@section('title', __('hr::lang.employee_salary_details'))

@section('content')

<!-- Content Header (Page header) -->
<section class="content-header">
    <h1>@lang('hr::lang.employee_salary_details')</h1>
</section>
            ...
            <div class="box box-primary">
                ...
                <form id="SalaryForm" action="{{action('\Modules\HR\Http\Controllers\SalaryController@store')}}"
                    ...
                    <div class="box-body">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control"
                                            name="comment"><?php if(!empty($empSalary->comment)) echo $empSalary->comment ?></textarea>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div class="col-md-3">
                                        <div class="row" style="padding-bottom: 15px">
                                            ...
                                            <div class="col-sm-6" id="resultTotalDeduction">
                                                <strong>@if(!empty($empSalary->total_deduction))
                                                    {{@number_format($empSalary->total_deduction)}} @else
                                                    {{@number_format(0)}} @endif</strong>
                                            </div>
                                            <input type="hidden" name="total_deduction" id="totalDeduction"
                                                value="@if(!empty($empSalary->total_deduction)) {{@number_format($empSalary->total_deduction)}}  @else {{@number_format(0) }} @endif ">
                                        </div>
                                        <div class="row" style="padding-bottom: 15px">
                                            <div class="col-sm-6">@lang('hr::lang.total_payable') :</div>
                                            <div class="col-sm-6" id="resultTotalPayable">
                                                <strong>@if(!empty($empSalary->total_payable))
                                                    {{@number_format($empSalary->total_payable)}} @else
                                                    {{@number_format(0)}} @endif</strong>
                                            </div>
                                            <input type="hidden" name="total_payable" id="totalPayable"
                                                value="@if(!empty($empSalary->total_payable)) {{@number_format($empSalary->total_payable)}}  @else  {{@number_format(0)}} @endif ">
                                        </div>
                                        <div class="row" style="padding-bottom: 15px">
                                            <div class="col-sm-6">@lang('hr::lang.cost_to_the_company') :</div>
                                            <div class="col-sm-6" id="resultCostToCompany">
                                                <strong>@if(!empty($empSalary->total_cost_company))
                                                    {{@number_format($empSalary->total_cost_company)}} @else
                                                    {{@number_format(0)}} @endif</strong>
                                            </div>
                                            <input type="hidden" name="total_cost_company" id="totalCostCompany"
                                                value="@if(!empty($empSalary->total_cost_company)) {{@number_format($empSalary->total_cost_company)}}  @else {{@number_format(0)}} @endif ">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        @lang('hr::lang.required_field')
                        <input type="hidden" name="id" id="emp_id" value="">
                        <input type="hidden" name="salary_id"
                            value="@if(!empty($empSalary->id)) {{$empSalary->id}} @endif">
                    </div>
                    <div class="box-footer">
                        <button id="saveSalary" type="submit" class="btn bg-olive btn-flat">@lang('hr::lang.save')
                        </button>
                    </div>
                </form>
